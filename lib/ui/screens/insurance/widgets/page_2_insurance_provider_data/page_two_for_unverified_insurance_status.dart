...
class PageTwoForUnVerifiedInsuranceStatus extends BaseStatefulWidget {
  const PageTwoForUnVerifiedInsuranceStatus({
    super.key,
  ...
  BaseState<BaseStatefulWidget> baseCreateState() {
    return _PageTwoForUnVerifiedInsuranceStatus();
  }
}

class _PageTwoForUnVerifiedInsuranceStatus
    extends BaseState<PageTwoForUnVerifiedInsuranceStatus>
    with validationBottomModelSheet, CancelInsuranceBottomModelSheet {
  ...
  Widget _getUnVerifiedErrorText() {
    return Expanded(
      child: Padding(
        ...
        child: Text(
          translate(LangKeys.yourAccountHasNotBeenValidated),
  ...
  Widget _getValidateButton() {
    return MultiBlocListener(
      ...
      child: InsurancePageButton(
        width: width * 0.3,
        onPressed: () {
          log("validate button Pressed to go to Validate");
          int? membershipNumber = int.tryParse(membershipNumberController.text);
          if (membershipNumber != null) {
            SendVerificationCodeBloc.bloc(context)
                .add(SendVerificationCodeEvent(membershipNumber));
          } else {
            showToast("Wrong Membership Number");
          }
        },
