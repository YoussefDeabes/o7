...
import 'package:o7therapy/ui/screens/checkout/widgets/payfort_payment_reschedule.dart';
...
class ReschedulePaymentScreen extends BaseScreenWidget {
  static const routeName = '/reschedule-payment-screen';

  const ReschedulePaymentScreen({Key? key}) : super(key: key);

  @override
  BaseState<ReschedulePaymentScreen> screenCreateState() =>
      _ReschedulePaymentScreenState();
}

class _ReschedulePaymentScreenState
    extends BaseScreenState<ReschedulePaymentScreen> {
  ...
  Widget _getBody() {
    return BlocConsumer<CheckoutBloc, CheckoutState>(
      listener: (context, state) {
        ...
        if (state is RescheduleSessionWithSavedCardSuccess) {
          cardModel.customerIp = state.ip;
          cardModel.sessionId = sessionId;
          cardModel.returnUrl = FortConstants.merchantExtra;
          cardModel.operationName = FortConstants.merchantExtraReschedule;
          context.read<CheckoutBloc>().add(PayNowWithSavedCardEvent(cardModel));
        }
        if (state is RescheduleSessionWithCardSuccess) {
          context
              .read<CheckoutBloc>()
              .add(PayNowWithSavedCardRescheduleEvent(cardModel));
        }
        if (state is PayWithSavedCardSuccess) {
          /// track mixpanel success reschedule event
          MixpanelBookingBloc.bloc(context)
              .add(SuccessfulSessionRescheduleEvent(
            newSessionId: sessionId,
            newSlotStringDateTimeInUtc:
                getUtcDateTimeFromBackEndLocalTimeString(sessionDate),
          ));
      ...
      builder: (context, state) {
        ...
        } else if (state is RescheduleSessionSuccess) {
          return PayfortPaymentRescheduleWidget(
            callback: (value) {
              if (value == 'success') {
                /// track mixpanel success reschedule event
                MixpanelBookingBloc.bloc(context)
          ...
          child: ElevatedButton(
            onPressed: () {
              if (!_formKey.currentState!.validate()) {
                ...
                context
                    .read<CheckoutBloc>()
                    .add(PayNowRescheduleEvent(int.parse(slotId), sessionId));
              }
            },
            style: ButtonStyle(
                shape: MaterialStateProperty.all<RoundedRectangleBorder>(
            ...
            onPressed: val != -1
                ? () {
                    context.read<CheckoutBloc>().add(
                        RescheduleSessionWithSavedCardEvent(
                            cardModel, sessionId, int.parse(slotId)));
                  }
                : () {},
            style: ButtonStyle(
                shape: MaterialStateProperty.all<RoundedRectangleBorder>(
